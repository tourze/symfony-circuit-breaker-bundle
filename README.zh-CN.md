...
Symfony 熔断器 Bundle，为 Symfony 应用提供高性能、可扩展的熔断器功能，
...
- [快速开始](#快速开始)
  ...
  - [直接使用服务](#直接使用服务)
  - [使用带熔断功能的 HTTP 客户端](#使用带熔断功能的-http-客户端)
- [配置](#配置)
  - [环境变量配置](#环境变量配置)
  - [配置参数说明](#配置参数说明)
...
- [存储后端](#存储后端)
  - [Redis 存储（推荐）](#redis-存储推荐)
  - [Doctrine 存储](#doctrine-存储)
  - [内存存储](#内存存储)
- [事件系统](#事件系统)
- [贡献指南](#贡献指南)
- [参考资料](#参考资料)
...
- `MINIMUM_NUMBER_OF_CALLS`: 最小调用次数
- `PERMITTED_NUMBER_OF_CALLS_IN_HALF_OPEN_STATE`: 半开状态下允许的调用次数
- `WAIT_DURATION_IN_OPEN_STATE`: 开启状态下的等待时间（秒）
- `SLIDING_WINDOW_SIZE`: 滑动窗口大小
- `SLOW_CALL_DURATION_THRESHOLD`: 慢调用时间阈值（毫秒）
...
- **Redis**: 用于高性能存储（推荐）
- **Doctrine DBAL**: 用于数据库存储降级
- **Symfony/HttpClient**: 用于 HTTP 客户端熔断器功能
...
class YourService
{
    ...
    {
        // 检查服务是否可用
        if (!$this->circuitBreaker->isAllowed('your.service')) {
            // 熔断器打开，执行降级逻辑
        ...
        try {
            ...
            $this->circuitBreaker->recordSuccess('your.service', $duration);
            
            return $result;
        } catch (\Throwable $e) {
            // 记录失败
            $duration = (microtime(true) - $startTime) * 1000;
            $this->circuitBreaker->recordFailure('your.service', $e, $duration);
...
Redis 存储是推荐的生产环境存储后端，支持原子操作和集群部署：
...
当 Redis 不可用时，自动降级到 Doctrine 存储：
...
熔断器会触发以下事件，你可以监听这些事件来实现自定义逻辑：

- `CircuitOpenedEvent`: 熔断器打开
- `CircuitClosedEvent`: 熔断器关闭
- `CircuitHalfOpenEvent`: 熔断器半开
- `CircuitSuccessEvent`: 调用成功
- `CircuitFailureEvent`: 调用失败
...
class CircuitBreakerListener
{
    public function onCircuitOpened(CircuitOpenedEvent $event): void
    {
        ...
        // 记录日志、发送告警等
        error_log("Circuit breaker opened for service: {$serviceName}");
    }
}
```
